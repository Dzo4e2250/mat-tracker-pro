...
/**
 * Get city name for a given postal code
 * @param postalCode - The postal code to look up
 * @returns The city name or empty string if not found
 */
export function getCityByPostalCode(postalCode: string): string {
  const trimmed = postalCode.trim();
  return POSTAL_CODES[trimmed] || '';
}

/**
 * Check if a postal code is valid (exists in the lookup)
 * @param postalCode - The postal code to validate
 * @returns true if valid, false otherwise
 */
export function isValidPostalCode(postalCode: string): boolean {
  return postalCode.trim() in POSTAL_CODES;
}
