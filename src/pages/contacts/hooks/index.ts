/**
 * Barrel export za contacts hooks
 */
export { useContactsFilters } from './useContactsFilters';
export { useOfferState } from './useOfferState';
export { useCompanyNotes } from './useCompanyNotes';
export { useQRScanner } from './useQRScanner';
export { useCompanyActions } from './useCompanyActions';
export { useOfferEmail, tableStyles, DEFAULT_TABLE_COLOR, buildTableStyles, formatWeeks } from './useOfferEmail';
export type { TextOverrides } from './useOfferEmail';
export { useContactSelection } from './useContactSelection';
export { useSentOffers } from './useSentOffers';
export { useCompanyDetailHandlers } from './useCompanyDetailHandlers';
export { useContactsModals } from './useContactsModals';
export { useRouteHelpers } from './useRouteHelpers';
export { useCompanyHierarchy } from './useCompanyHierarchy';
export { useBusinessCardScanner } from './useBusinessCardScanner';

// Export types
export type { UseCompanyNotesReturn } from './useCompanyNotes';
export type { UseOfferStateReturn } from './useOfferState';
export type { UseCompanyActionsReturn } from './useCompanyActions';
export type { UseCompanyDetailHandlersReturn } from './useCompanyDetailHandlers';
export type { UseSentOffersReturn } from './useSentOffers';
export type { UseOfferEmailReturn } from './useOfferEmail';
export type { UseQRScannerReturn } from './useQRScanner';
export type { UseBusinessCardScannerReturn } from './useBusinessCardScanner';
